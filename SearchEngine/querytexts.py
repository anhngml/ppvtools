...
class Query:
    ...
    def __init__(self, filenames, reindex):
        indexfile = os.path.join('TextSearchEngine', 'indexer.npz')
        ...
        if reindex:
            ...
            with np.load(indexfile) as data:
                saved_data = data['data'][()]
                self.index = saved_data['index']
                self.filenames = self.index.filenames
                self.invertedIndex = self.index.totalIndex
                self.regularIndex = self.index.regdex
